<x-layouts.guest subtitle="Crie sua conta para acessar o painel do cliente">
    <form method="POST" action="{{ url('/app/register') }}" class="space-y-4">
        ...
        <div>
            ...
            <input id="name" name="name" type="text" required autofocus value="{{ old('name') }}" class="w-full rounded-md border border-neutral-300 px-3 py-2 outline-none focus:border-primary-500">
        </div>

        <div>
            <label for="email" class="mb-1 block text-sm font-medium">E-mail</label>
            <input id="email" name="email" type="email" required value="{{ old('email') }}" class="w-full rounded-md border border-neutral-300 px-3 py-2 outline-none focus:border-primary-500">
        </div>
    ...
    <div class="mt-5 text-sm">
        <a href="{{ route('login') }}" class="text-neutral-700 underline">Já tenho conta</a>
    </div>
</x-layouts.guest>
